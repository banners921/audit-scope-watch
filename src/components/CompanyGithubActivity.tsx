    ...
    queryFn: async () => {
      ...
      await Promise.all(
        repos.map(async (full) => {
          try {
            const url = `https://api.github.com/repos/${full}/commits?since=${encodeURIComponent(since)}&per_page=100`;
            ...
            for (const c of arr) {
              ...
              if (d && (!lastCommitDate || d > lastCommitDate)) lastCommitDate = d;
            }
          } catch {
            /* ignore */
          }
      ...
      return { totalCommits, lastCommitDate, reposCount: reposWithData || repos.length };
    },
  });
      ...
      {q.isLoading ? (
        ...
        <div className="space-y-2 text-sm">
          <div className="font-mono text-lg text-white">
            {q.data.totalCommits} commits across {q.data.reposCount} repo{q.data.reposCount === 1 ? "" : "s"} in last {days} days
          </div>
